...
## Choose a collaboration operation
...
6. Record `work.started` for new work or `work.resumed` for paused or revising
   work, only after the actor exists and owns the assignment.
...
whether its accumulated context will help the next responsibility:
...
Keep agents for useful context, not to maintain a standing team.

## Integrate a returned result
...
A returned result needs recorded review before it can be accepted.
