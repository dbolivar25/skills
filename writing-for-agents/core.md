# Writing for agents core

Read the [core/shell contract](../contracts/core-shell.md).

**Inputs:** Intended behavioral change, current/proposed instructions, consumers, invocation context, host mechanics and existing evidence; optional requested artifact format.
...
A no-op conclusion requires supplied comparison runs with and without the
instruction. Request that evidence for a consequential uncertainty. If it changes
...
Finish by assessing relevance and behavior against supplied evidence. Remove **sediment**: stale facts,
unrelated exposition, and branches that no longer serve the task. Compare the
proposed revision with the original intent on representative cases, especially where
...
[evaluation design](../evaluation-design/core.md) with the behavior it should
change, representative caller tasks and failure consequences. Return the comparison plan as an evidence need when results are required; a valid description or a shorter
