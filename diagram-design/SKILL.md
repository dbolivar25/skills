---
name: diagram-design
description: Use when a relationship, flow, state, hierarchy, or dataset needs a polished standalone diagram, or an existing Draw.io, Mermaid, or Excalidraw source needs redrawing. Load it to select the right semantic pattern and visual type, apply an onboarded brand skin, and produce and visually verify an accessible HTML, SVG, or PNG artifact; it also supports explanatory motion and sketchy styling.
license: MIT
metadata:
  version: "2.6"
---
...
| JetBrains Mono as blanket "dev" font | Mono is for *technical* content — ports, commands, URLs. Names use the active `node-name` family. |
...
| Accent on every "important" node | Accent is reserved for 1–2 editorial focal elements, not a signaling system |
...
**The design system is skinnable.** All colors, typography, and tokens live in a single source of truth — [`references/style-guide.md`](references/style-guide.md). This file describes semantic roles (`paper`, `ink`, `muted`, `accent`, `link`, …). The default skin is a cool editorial palette; the active working copy may be branded. Apply the active style guide to every diagram. When a hard-coded value or font in a type reference or example differs from the active guide, treat it as a shipped-default illustration and substitute the active semantic token or type role.
...
- **Title** — Matter SQ, 1.75rem, 400 — H1 only
- **Node name** — Matter SQ, 12px, 600 — human-readable labels
- **Sublabel** — system mono, 9px — ports, URLs, field types
- **Eyebrow / tag** — Matter SQ, 12px, 500, sentence case — type tags, axis labels
- **Arrow label** — Matter SQ, 12px — human-readable annotations; technical values use mono
- **Editorial aside** — Matter SQ *italic*, 14px — callouts only

**CJK labels** — Matter SQ carries no Hangul or Han; extend the family and keep CJK at 12px+. Rules: [Korean](references/style-guide.md#korean-labels), [Chinese](references/style-guide.md#traditional-chinese-labels).

**Mono is for technical content only** — never as a blanket "dev" font, and never JetBrains Mono.

Use the packaged Matter SQ sources and exact `@font-face` declarations in the
active [style guide](references/style-guide.md#font-sources).
...
```svg
<rect width="100%" height="100%" fill="#f6f1ef"/>
```
...
<defs>
  <pattern id="dots" width="22" height="22" patternUnits="userSpaceOnUse">
    <circle cx="1" cy="1" r="0.9" fill="rgba(4,9,25,0.10)"/>
  </pattern>
</defs>
<rect width="100%" height="100%" fill="#f6f1ef"/>
...
<marker id="arrow" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#2b2d39"/>
</marker>
<marker id="arrow-accent" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#660033"/>
</marker>
<marker id="arrow-link" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#6f7288"/>
</marker>
```

| Arrow | Stroke | When |
|---|---|---|
| Default | muted `#2b2d39` | Internal, generic |
| Accent | burgundy `#660033` | Primary / highlighted / headline |
| Link | cloud-burst `#6f7288` | HTTP/API calls, external systems |
...
<rect x="X" y="Y" width="W" height="H" rx="6" fill="#f6f1ef"/>
<!-- 2. Styled box -->
...
<text x="X+22" y="Y+15" fill="STROKE@0.8" font-size="12" font-weight="500"
      font-family="'Matter SQ', sans-serif" text-anchor="middle">API</text>
<!-- 4. Node name (Matter SQ — human-readable) -->
<text x="CX" y="CY+2" fill="#040919" font-size="12" font-weight="600"
      font-family="'Matter SQ', sans-serif" text-anchor="middle">Node name</text>
<!-- 5. Technical sublabel (system mono) -->
<text x="CX" y="CY+18" fill="#6f7288" font-size="9"
      font-family="ui-monospace, 'SF Mono', Menlo, monospace" text-anchor="middle">tech:port</text>
```
...
```svg
<!-- Mask sits 14px above the arrow (8px visible type + 6px gap). Stroke is at ARROW_Y. -->
<rect x="MID_X-26" y="ARROW_Y-26" width="52" height="20" rx="2" fill="#f6f1ef"/>
<text x="MID_X" y="ARROW_Y-12" fill="#6f7288" font-size="12"
      font-family="'Matter SQ', sans-serif" text-anchor="middle">Write</text>
```

Rules:

- ≤14 characters, sentence case, centered on segment midpoint.
...
<line x1="30" y1="LEGEND_Y-8" x2="VIEWBOX_W-30" y2="LEGEND_Y-8"
      stroke="rgba(4,9,25,0.12)" stroke-width="0.8"/>
<text x="30" y="LEGEND_Y+8" fill="#2b2d39" font-size="12" font-family="'Matter SQ', sans-serif">Legend</text>
...
1. **Header** — sentence-case label, title, and optional subtitle in the active typography roles.
...
4. **Footer** — technical colophon in the active mono family, muted, hairline top border.
...
- `border: 1px solid rgba(4,9,25,0.12)`
...
- [ ] Every arrow label has an opaque rect filled with the active `paper` token behind it?
...
- [ ] Human-readable names use the active `node-name` family, not mono?
- [ ] Technical sublabels (ports, commands, URLs) use the active mono family?
- [ ] Page title uses the active `title` family?
- [ ] Annotation callouts (if any) use the active italic `callout` family? (see [primitive-annotation.md](references/primitive-annotation.md))
