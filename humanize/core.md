...
**Inputs:** Source prose, requested mode, audience/genre, explicit voice sample or authority, protected facts and structures, requested output format.
...
For a file target, propose changes only to the authorized prose. In embedded mode, return only
...
invariants. A file edit is a proposed replacement; the shell owns writing it.
Return an evidence or owner-decision need for substantive uncertainty that cannot
be repaired without new facts. Embedded use returns only the requested prose.
Do not expose an internal drafting ceremony unless requested.
