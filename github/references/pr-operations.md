...
A complete state snapshot and the substantive review conversation answer different
questions. Record missing pages, truncation and stale heads as coverage limits.
Unresolved/resolved status is platform state; inspect current code to determine
whether the underlying issue remains.

Pending CI is not failing CI. Retrieval success means the report was acquired,
not that its checks passed. Keep command exit status, observation status and
check conclusions distinct. An old head or local checkout cannot establish the
current remote comparison.

Return the supported facts, scope, currentness and exact missing receipt. Resolve
obtainable gaps through the appropriate query or helper. A merge recommendation
belongs to the review caller; favorable facts do not authorize a live action.
...
A reply does not resolve a thread. A drafted review is not publication, and an
accepted request without read-back is not a verified resulting state. Afterward,
report the action, target, resulting state and any comment that could not be
anchored or verified.
