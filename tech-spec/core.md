...
**Inputs:** Selected problem, current-system evidence, callers, goals/non-goals, constraints, invariants, owner choices, optional alternatives and expected acceptance behavior.

Deliver a **typed call-stack architecture handoff**: contracts in code and complete
execution flows, with prose explaining why. The core returns a design artifact; it neither writes the file nor implements it.

## 1. Establish the design problem

Read [engineering standards](../software-engineering/references/standards.md),
[module design](../module-design/core.md),
[testing evidence](../software-engineering/references/testing-evidence.md) and
[vertical test slices](../software-engineering/references/testing-evidence.md) as planning
reference, not implementation authority. Use supplied local code/docs for vocabulary, layout, domain concepts, errors,
adapters, observability, runtime patterns and test style. Missing source or an
unresolved requirement returns an evidence or owner-decision need.
...
Sufficient context permits direct design; a scan is not a prerequisite. Return
missing facts or unresolved behavior precisely enough for the shell to acquire
or ask without inventing requirements. The result remains the typed handoff,
not merely a list of questions.
...
operational fit and implementation complexity. When independently produced alternatives are supplied, preserve their distinct
briefs and evidence in the comparison. The shell owns any selected exploration.
...
missing decisions. Return the typed design and any unresolved decisions; the shell owns delivery.
