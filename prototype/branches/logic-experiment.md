...
If the question is "what should this look like" — wrong branch. Use [UI experiment](ui-experiment.md).
...
Put the actual logic — the bit that's answering the question — in a single `<script>` block written as a small, pure module that could be lifted out and dropped into the real codebase later. Keep the logic portable so it can be evaluated separately from the page. Both remain prototype work; reuse requires implementation authority and normal production checks.
