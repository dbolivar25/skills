...
**Inputs:** Consumer question, actual/prior source versions, intended behavior,
meaningful changed regions, source context and validation receipts; review criteria only
for a review caller.

Build a grounded model of an existing change, proposal, or work product before judging
it or explaining it to another person. The caller supplies the source and the question
or decision the map must support. Return a current change map with evidence and gaps;
this method neither issues a verdict nor starts a writing workflow. A caller may supply
a current map; recheck changed sources and fill its gaps rather than repeating sound
investigation.
...
For code judgment, the caller supplies the target, governing standards, local precedent
and review axes. For a PR, inputs identify base/head, changed regions, commit/discussion
...
transition or source outside the supplied excerpt, return the exact evidence need and
affected conclusion.

Completion: every material region has an understood role or is explicitly unexplained.
Every validation claim is supported by a supplied receipt or marked unverified. No claim
