# Dependency compatibility core

Read the [core/shell contract](../contracts/core-shell.md).

**Inputs:** Exact old/new versions, resolved lock graph, runtime envelope, repository usage evidence and primary upstream release/migration records for the interval.
...
From the supplied component, resolution and runtime records, establish:
...
A bot-authored summary cannot substitute for the lock graph and runtime records.
Return missing exact versions, resolution or platform facts as evidence needs.
...
Account for supplied evidence of every relevant surface, not only imports:
...
metadata, and supported API documentation. Compare the supplied installed or resolved version with those sources; an
unknown version or missing intermediate release is an evidence gap.
...
requested version and migration scope. Unknown target versions remain explicit evidence needs, never guesses.
