...
**Inputs:** User moment, supplied surface/source/render observations, platform/category
and brand constraints, states/viewport context, chosen direction and task mode (design
or assessment).
...
- Use supplied renders, screenshots, mockups, prototype observations and source
  behavior. Missing real-surface evidence returns a specific render/state need;
  do not prescribe polish from a generic mental picture.
...
2. **Choose the working mode**
   ...
   If the question is about feel, return a breakable-toy or live-tuning
   experiment design. Its proposed controls expose duration, easing, spacing, blur, shadow, position, scale,
   rotation, offsets, or generative parameters through temporary controls instead
   of guessing through repeated edits. If the mechanism is unknown, return the
   smallest discriminating experiment and the observation needed to understand it.
...
5. **Resolve layers and dynamics**
   ...
   Treat rendering mechanics as part of the design. Use supplied layer and paint
   behavior evidence before judging decoration; request it when consequential. Map each changing input or state onto visible
...
7. **Intervene and verify**
   ...
   Propose the smallest change that resolves the named issue. For implementation
   assessment, require supplied browser/platform observations across relevant
   states and viewports. For critique, ground every finding in an observation, explain the
...
For supplied implementation evidence, return:

- The unresolved issue you addressed.
- The concrete changes established by the supplied source.
- The supplied verification, including states, viewport, browser, or behavior
  checked, and what remains unverified.
- Any remaining risk.

For critique, lead with findings ordered by impact. Each finding should use this shape:
observation, why it matters to the moment, and what direction would resolve it. Do not
...
If the work is not ready, say what evidence is missing: real surface, content, states,
platform, category floor, motion behavior, performance behavior, or user context.
...
Use [decision rights](../decision-rights/SKILL.md) for a proposed objection with
supplied owner/authority evidence. For a needed interaction experiment, use
[experiment design](../experiment-design/SKILL.md) with the unresolved question and
constraints. Return the experiment and the observations it needs; the caller owns
construction, execution and any implementation changes.
