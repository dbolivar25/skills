...
When connection-pool exhaustion is a ranked hypothesis, read
[`references/connection-pool-exhaustion.md`](references/connection-pool-exhaustion.md)
for its current-configuration gate, predictions, and alternative causes. Use
`grafana-evidence` when production telemetry is needed to test those predictions.
