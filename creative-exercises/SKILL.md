...
Concrete exercises grouped by duration. Use when the user wants to *do* an exercise, not be given ideas. Each entry names its parent method and expected output. Load `creative-ideation`, then use the named method or resource from that loaded skill; do not resolve sibling filesystem paths from this skill.
...
**TRIZ contradiction analysis** *(`creative-ideation`: `triz-principles`)* — state problem as contradiction (improving X degrades Y); look up 2–3 candidate principles; for each, generate one mechanism in your specific case; pick the strongest. Output: contradiction statement + 1 elaborated mechanism.

**James Webb Young, compressed** *(`creative-ideation`: `volume-generation`)* — gather specific material (15min) → digest, make connections (15min) → walk away (10min) → idea arrives (variable) → shape (20min). Output: a written idea that has been incubated.

**Affinity diagram** *(`creative-ideation`: `affinity-diagrams`)* — write each note/quote on its own card; spread them out; cluster silently; name each cluster; note orphans and gaps. Output: bottom-up taxonomy + list of gaps.

**Sol LeWitt instruction** *(`creative-ideation`: `creative-discipline`)* — define the work as an instruction not an object; write it as a single sentence; the work is the instruction. Optionally execute it once. Output: an instruction-as-work.

## 1 day

**Tharp's box** *(`creative-ideation`: `creative-discipline`)* — get a literal box; spend the day collecting everything related to your project (clippings, references, sketches, sources, objects); label it; keep adding for the project's duration. Output: physical archive + practice of returning.
...
**Compression-progress research week** *(`creative-ideation`: `compression-progress`)* — Day 1–2: identify a domain you have weak predictions in. Day 3–5: read deeply. Day 6: write the new patterns you can predict. Day 7: pick the question whose answer would most compress your model further. Output: a research question grounded in your current model.

**Pattern-language week** *(`creative-ideation`: `pattern-languages`)* — Day 1–2: identify ten recurring problems. Day 3–4: write each as a pattern (context, problem, generative solution). Day 5: arrange in partial order. Day 6: design using the patterns as vocabulary. Day 7: review. Output: a small pattern language and a design that uses it.

**Cleese open-mode week** *(`creative-ideation`: `creative-discipline`)* — each day: protect 90 minutes during which you do nothing useful, don't check messages, don't finish anything. The work is to not be in closed mode. Output: not an idea — the conditions for ideas.

## Multi-week

**Cameron's *Artist's Way* (12 weeks)** *(`creative-ideation`: `creative-discipline`)* — daily morning pages (3 longhand pages, stream of consciousness, don't reread for 8 weeks). Weekly artist date (2 hours solo, doing something that interests you). Output: a different relationship to the work.

**Lynda Barry image-bath** *(`creative-ideation`: `creative-discipline`)* — daily for several weeks: list 10 things you saw today; pick one; draw it (badly is fine); write a paragraph from inside the memory it surfaces. Output: an archive of recovered specifics.
